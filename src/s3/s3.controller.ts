...
export class S3Controller {
  ...
  async deleteFile(@Param('key') key: string) {
    return await this.s3Service.deleteFile(key); // Fixed: was calling itself recursively
  }
  ...
  async getSignedUploadUrl(@UploadedFile() file: Express.Multer.File) {
    const key = this.generateKey(file);
      return await this.s3Service.getSignedUploadUrl(key, 300, file.mimetype)
    };
  ...
  async getSignedDeleteUrl(@Param('key') key: string) {
    return await this.s3Service.getSignedDeleteUrl(key, 300)
  }
}